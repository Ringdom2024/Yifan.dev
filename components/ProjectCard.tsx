...
const ProjectCard: React.FC<Props> = ({ index, project }) => {
  ...
  useEffect(() => {
    const githubApi: string = project.githubApi ?? "";
    if (!githubApi) return;
  ...
  return (
    <div ref={sectionRef} className={`md:basis-1/2 md:px-8 py-2 md:py-4`}>
      <div className={`project-card project-card-${index}`}>
            ...
            <div className="flex items-center space-x-5 sm:space-x-3 my-2 sm:my-0 mr-[0.1rem]">
              {starCountUrl && (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  title={`${t("project.stars")} ${project.title}`}
              ...
              {project.codeUrl && (
                <a
                  ...
                  title={`${t("project.code")} ${project.title}`}
                  target="_blank"
                  rel="noreferrer"
              ...
              {project.liveUrl && (
                <a
                  ...
                  title={`${t("project.demo")} ${project.title}`}
                  target="_blank"
                  rel="noreferrer"
        ...
        <ul
          aria-label={`${t("project.stack")} ${project.title}`}
